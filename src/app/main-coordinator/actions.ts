...
export async function getDepartments(): Promise<Department[]> {
  try {
    return await queryDb<Department>(`SELECT department_id, department_name FROM public.department_master ORDER BY department_name`);
  } catch (error) {
...
> {
  ...
  try {
    ...
    for (const row of rows) {
      if (!result[row.offering_id]) result[row.offering_id] = [];
      result[row.offering_id].push(row);
    }
    return result;
  } catch (error) {
    console.error("Failed to fetch SOC components:", error);
    return {};
  }
}

export async function getDeptOfferingsForSoc(faculty_id: number): Promise<{
  department_id: string;
  department_name: string;
  offering_id: string | null;
}[]> {
  try {
    // Get all offering_ids assigned to this SOC coordinator
    const assignments = await queryDb<{ offering_id: string }>(
      `SELECT offering_id FROM public.main_coordinator_assignment WHERE faculty_id = $1`,
      [faculty_id]
    );
    const offeringIds = assignments.map(a => a.offering_id);

    if (offeringIds.length === 0) {
      const depts = await queryDb<Department>(`SELECT department_id, department_name FROM public.department_master ORDER BY department_name`);
      return depts.map(d => ({ ...d, offering_id: null }));
    }

    // For each dept, find if there's a dept_coordinator_assignment for this offering in that dept
    return await queryDb<{ department_id: string; department_name: string; offering_id: string | null }>(`
      ...
      LEFT JOIN public.dept_coordinator_assignment dca
        ...
        AND dca.offering_id = ANY($1::uuid[])
      ORDER BY dm.department_id, dm.department_name
    `, [offeringIds as unknown as string]);
  } catch (error) {
    console.error("Failed to fetch dept offerings for SOC:", error);
    return [];
  }
}

export async function getFacultiesByDept(dept_id: string): Promise<DeptFaculty[]> {
  try {
    return await queryDb<DeptFaculty>(
      `SELECT faculty_id, faculty_name, designation, email 
       FROM public.faculty 
    ...
    console.error("Failed to fetch faculties for dept:", error);
    return [];
  }
}

export async function getFacultyOfferings(faculty_id: number): Promise<CoordinatorOffering[]> {
  try {
    return await queryDb<CoordinatorOffering>(`
      ...
      ORDER BY ay.year_name DESC, sm.semester_name, cm.course_name
    `, [faculty_id]);
  } catch (error) {
    console.error("Failed to fetch faculty offerings:", error);
    return [];
  }
}
