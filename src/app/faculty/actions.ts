...
export type FacultyCommonComponent = {
  course_component_id: string;
  offering_id: string;
  component_name: string;
  course_code: string;
  course_name: string;
  common_file_key: string;
  common_file_name: string;
  uploaded_by_name: string | null;
  common_uploaded_at: string | null;
};

/**
 * Common components (one shared file per component, uploaded by the coordinator)
 * for the courses this faculty is assigned to. View-only for faculty.
 */
export async function getFacultyCommonComponents(
  faculty_id: number
): Promise<FacultyCommonComponent[]> {
  return queryDb<FacultyCommonComponent>(`
    SELECT DISTINCT
      ...
      cc.common_file_name,
      f.faculty_name AS uploaded_by_name,
      cc.common_uploaded_at
    FROM public.course_component cc
    JOIN public.component_master cmp ON cc.component_id = cmp.component_id
    JOIN public.course_offering co ON cc.offering_id = co.offering_id
    JOIN public.course_master cm ON co.course_id = cm.course_id
    JOIN public.faculty_assignment fa ON fa.offering_id = cc.offering_id
    LEFT JOIN public.faculty f ON cc.common_uploaded_by = f.faculty_id
    WHERE fa.faculty_id = $1 AND cc.is_common = true AND cc.common_file_key IS NOT NULL
    ORDER BY cm.course_code, cmp.component_name
  `, [faculty_id]);
}
