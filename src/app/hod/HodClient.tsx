...
function groupRows(rows: HodDetailedSubmission[]): CourseGroup[] {
  ...
  return Array.from(courseMap.values()).map(({ meta, facultyMap }) => {
    ...
    const faculties = Array.from(facultyMap.values()).map(({ meta: fMeta, compMap }) => {
      ...
      const fPendCount = components.filter((c) => c.status === "pending" || c.status === null).length;
      ...
      {open && (
        <div className="border-t border-black/5">
          {course.faculties.length === 0 ? (
            ...
            course.faculties.map((faculty) => <FacultyCard key={faculty.assignment_id} group={faculty} />)
          )}
        </div>
      )}
    </div>
...
export default function HodClient({
  ...
  initialData: HodDetailedSubmission[];
  stats: HodDeptStats;
}) {
  const [searchTerm, setSearchTerm] = useState("");

  const courseGroups = useMemo(() => groupRows(initialData), [initialData]);
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {filteredCourses.length === 0 ? (
            ...
            filteredCourses.map((course) => <CourseCard key={course.offering_id} course={course} />)
          )}
        </div>
      </div>
    </div>
