// Secondary coordinator dashboard: shows all course offerings they are assigned to.
...
export default async function SecondaryCoordinatorPage() {
  ...
  if (offerings.length === 1) {
    redirect(`/secondary-coordinator/${offerings[0].offering_id}`);
  }
      ...
      {offerings.length === 0 ? (
        <div className="panel-card border-dashed border-gray-300 p-5 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Ask an Admin to assign you as secondary coordinator to a course offering.
          </p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {offerings.map((offering) => (
            <Link
              ...
              href={`/secondary-coordinator/${offering.offering_id}`}
